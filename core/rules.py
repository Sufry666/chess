...
def is_cantMove(board_present, color): # 判断color方是否无法移动 若无法移动则返回True
    for r_idx, row_cells in enumerate(board_present):
        for c_idx, cell in enumerate(row_cells):
            if cell == 0 or cell.color != color:
                continue
            piece = cell
            if len(piece.get_possible_moves(board_present)) != 0:
                return False
    return True

def is_Checkmate(board_present, king_position, color): #判断color方是否被将杀 若被将杀返回True
    if is_cantMove(board_present, color) and isChessmate(board_present, king_position):
        return True
    return False

def is_Stalemate(board_present, king_position, color): #判断color方是否被逼和 若被逼和返回True
    if is_cantMove(board_present, color) and (not isChessmate(board_present, king_position)):
        return True
    return False
