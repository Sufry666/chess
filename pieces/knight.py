...
def main():
    row, col = 2,4
    knight_piece = knight("white", position=(row, col))
    knight_piece_1 = knight("white", position=(row+1, col+2))
    board_test = [[0 for _ in range(8)] for _ in range(8)]
    board_test[row][col] = knight_piece
    board_test[row+1][col+2] = knight_piece_1
